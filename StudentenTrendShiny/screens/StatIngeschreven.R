            ...
            fluidRow(
              ...
              tabBox(width=12, height=650, 
                       tabPanel("Studieniveau",
                          ...
                          plotlyOutput("Ingeschreven_HBOB_WOB", height = 450)
                        ),
                        tabPanel("Geslacht",
                          h1("Chikwadraat"),
                          ...
                          plotlyOutput("Ingeschreven_Man_Vrouw", height = 450)      
                        ) 
              )
            )
    )   
...
StatIngeschrevenServer <- function(input,output, session){
  ...
  output$Ingeschreven_HBOB_WOB <- renderPlotly({
  ...
  output$Ingeschreven_Man_Vrouw <- renderPlotly({
...
MakeStatGGPlotly <- function(plot) {
  PrintGGPlotly(plot, margin=list(r=130, b=150))
}